function Cluster() {
  ...
  function applyClusterRecoveringState() {
    ...
    var recoverButtons = $("button[id^='recover_dropdown_']");
    if (_clusterHasActiveRecovery) {
  ...
  function domReady() {
    ...
    getData("/api/active-cluster-recovery/" + currentClusterName(), function(recoveries) {
      ...
      recoveries.forEach(function(recoveryEntry) {
        addInfo('<strong><a href="' + appUrl('/web/audit-recovery-active/cluster/' + encodeURIComponent(currentClusterName())) + '">' + escapeHtml(recoveryEntry.AnalysisEntry.Analysis) + ' active recovery in progress</strong></a>. Topology is subject to change in the next moments.');
      });
    });
